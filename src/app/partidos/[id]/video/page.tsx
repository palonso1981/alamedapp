...
import { MatchEvent, MatchSession, MatchVideoAnchor, MatchVideoPeriod, MatchVideoSegment } from "../../../../types";
...
export default function MatchVideoPage() {
  ...
  function addSegment() {
    try {
      ...
      setUrl(""); setLabel(""); setMessage(null); setAdding(false);
  ...
  return <div className="min-h-screen bg-slate-900 text-white"><AppHeader title="Vídeo del partido" clubId={preparation?.clubId} actions={<SyncStatusBadge matchId={matchId}/>} /><main className="mx-auto max-w-5xl space-y-4 p-4 sm:p-6">
    ...
    <section className="rounded-3xl border border-slate-700 bg-slate-950 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black tracking-widest text-slate-500">0..N SEGMENTOS INDEPENDIENTES</p><h2 className="text-xl font-black">VÍDEOS DEL PARTIDO</h2></div><button type="button" onClick={() => setAdding((value) => !value)} className="min-h-12 rounded-xl bg-red-600 px-5 text-xs font-black">{adding ? "CANCELAR" : "+ AÑADIR VÍDEO"}</button></div><div className="mt-4 space-y-4">{(session.videoSegments ?? []).map((segment) => <SegmentCard key={segment.id} segment={segment} session={session} candidates={candidates} onCommit={commit} onMessage={setMessage}/>)}</div>{(session.videoSegments ?? []).length === 0 && <p className="mt-4 rounded-2xl border border-dashed border-slate-700 p-8 text-center text-sm text-slate-400">Este partido todavía no tiene vídeos. Puedes añadir un vídeo completo o segmentos independientes para P1 y P2.</p>}</section>
    {adding && <section className="grid gap-3 rounded-3xl border border-red-900 bg-slate-950 p-4 sm:grid-cols-[1fr_1fr_auto]">
      <h2 className="text-sm font-black text-red-300 sm:col-span-3">NUEVO VÍDEO</h2>
      ...
      <label className="text-[9px] font-black text-slate-400">NOMBRE DEL VÍDEO <span className="font-normal">· solo para identificarlo</span><input value={label} onChange={(event) => setLabel(event.target.value)} placeholder={coverage === "P1" ? "1ª parte (automático)" : coverage === "P2" ? "2ª parte (automático)" : "Partido completo (automático)"} className="mt-1 min-h-12 w-full rounded-xl bg-slate-800 px-3 text-sm"/></label>
...
function SegmentCard({ segment, session, candidates, onCommit, onMessage }: { segment: MatchVideoSegment; session: MatchSession; candidates: MatchEvent[]; onCommit: (session: MatchSession) => void; onMessage: (message: string | null) => void }) {
  ...
  const [coverage, setCoverage] = useState<"P1" | "P2" | "FULL">(segment.periods.length === 2 ? "FULL" : segment.periods[0] === 1 ? "P1" : "P2");
  ...
  function saveDetails() {
    ...
    const periods: MatchVideoPeriod[] = coverage === "FULL" ? [1, 2] : [coverage === "P1" ? 1 : 2];
    const compatibleAnchors = segment.anchors.filter((anchor) => { const event = session.events.find((candidate) => candidate.id === anchor.eventId); return event && periods.includes(event.period as MatchVideoPeriod); });
    if (videoId === segment.videoId && compatibleAnchors.length !== segment.anchors.length && !window.confirm("Cambiar la cobertura eliminará las anchors que queden fuera. ¿Continuar?")) return;
    onCommit(upsertVideoSegment(session, { ...segment, videoId, periods, anchors: compatibleAnchors, label: label.trim() || segment.label, leadSeconds: lead, updatedAt: Date.now() }));
  ...
  function correctAnchor(anchor: MatchVideoAnchor) {
    const value = window.prompt("Nuevo tiempo de vídeo (mm:ss)", formatVideoTimestamp(anchor.videoSecond));
    if (value === null) return;
    const second = parseVideoTimestamp(value);
    if (second === null) return onMessage("El tiempo de vídeo no es válido.");
    try { onCommit(addVideoAnchor(session, segment.id, { ...anchor, videoSecond: second })); onMessage(null); }
    catch (error) { onMessage(error instanceof Error ? error.message : "No se pudo corregir el anchor."); }
  }
  const probeEvent = segment.anchors[0] ? session.events.find((event) => event.id === segment.anchors[0].eventId) : undefined;
  const probe = probeEvent ? resolveEventVideoPosition(session, probeEvent.id) : null;
  const needsReview = probe?.status === "RESOLVED" && probe.quality === "MULTI_ANCHOR_WARNING";
  return <section className="rounded-3xl border border-slate-700 bg-slate-800 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black text-red-300">YOUTUBE · {segment.periods.map((period) => `P${period}`).join(" + ")} · −{segment.leadSeconds}s</p><h3 className="text-xl font-black">{segment.label}</h3><p className={`mt-1 text-[10px] font-black ${needsReview ? "text-amber-300" : segment.anchors.length ? "text-emerald-300" : "text-amber-300"}`}>{needsReview ? "REVISAR SINCRONIZACIÓN" : segment.anchors.length ? "SINCRONIZADO" : "SINCRONIZACIÓN PENDIENTE"} · {segment.anchors.length} {segment.anchors.length === 1 ? "ANCLA" : "ANCLAS"}</p><code className="text-[10px] text-slate-500">{segment.videoId}</code></div><div className="flex flex-wrap justify-end gap-2">{probe?.status === "RESOLVED" && <a href={probe.url} target="_blank" rel="noopener noreferrer" className="inline-grid min-h-11 place-items-center rounded-xl bg-red-600 px-3 text-xs font-black">▶ PROBAR</a>}<button type="button" onClick={() => setSynchronizing((value) => !value)} className="min-h-11 rounded-xl bg-amber-400 px-3 text-xs font-black text-slate-950">{segment.anchors.length ? "+ OTRA ANCLA" : "SINCRONIZAR"}</button><button type="button" onClick={() => setEditing(!editing)} className="min-h-11 rounded-xl bg-slate-700 px-3 text-xs font-black">EDITAR</button><button type="button" onClick={() => { if (window.confirm("¿Eliminar este índice de vídeo? Los eventos no se modificarán.")) onCommit(removeVideoSegment(session, segment.id)); }} className="min-h-11 rounded-xl bg-slate-700 px-3 text-xs font-black">ELIMINAR</button></div></div>
    {editing && <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_1fr_7rem_6rem_auto]"><input aria-label="URL o ID editado" value={url} onChange={(event) => setUrl(event.target.value)} className="min-h-11 rounded-xl bg-slate-950 px-3 text-sm"/><input aria-label="Nombre del vídeo editado" value={label} onChange={(event) => setLabel(event.target.value)} className="min-h-11 rounded-xl bg-slate-950 px-3 text-sm"/><select aria-label="Cobertura editada" value={coverage} onChange={(event) => setCoverage(event.target.value as typeof coverage)} className="min-h-11 rounded-xl bg-slate-950 px-2 text-xs"><option value="P1">P1</option><option value="P2">P2</option><option value="FULL">P1 + P2</option></select><input aria-label="Margen editado" type="number" min={0} max={20} value={lead} onChange={(event) => setLead(Number(event.target.value))} className="min-h-11 rounded-xl bg-slate-950 px-3"/><button type="button" onClick={saveDetails} className="min-h-11 rounded-xl bg-cyan-400 px-4 text-xs font-black text-slate-950">GUARDAR</button></div>}
    {synchronizing && <div className="mt-4 rounded-2xl bg-slate-900 p-3"><p className="text-[10px] font-black text-cyan-300">SINCRONIZAR CON UNA JUGADA</p><p className="mt-1 text-[10px] text-slate-500">Los goles aparecen como RECOMENDADOS. También puedes elegir cualquier acción capturada en directo que reconozcas bien.</p><div className="mt-2 grid gap-2 sm:grid-cols-[1fr_2fr_8rem_auto]"><input aria-label="Buscar evento" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar jugador, minuto…" className="min-h-11 rounded-xl bg-slate-950 px-3 text-sm"/><select aria-label="Evento anchor" value={eventId} onChange={(event) => setEventId(event.target.value)} className="min-h-11 min-w-0 rounded-xl bg-slate-950 px-3 text-xs"><option value="">ELIGE JUGADA</option>{eligible.map((event) => <option key={event.id} value={event.id}>{event.type === "threat_recorded" && event.outcome === "GOL" ? "★ RECOMENDADO · " : ""}P{event.period} · min {event.minute} · {eventDescription(event, session.players)} · Registrado {captureTime(event)}</option>)}</select><input aria-label="Tiempo de vídeo" value={timestamp} onChange={(event) => setTimestamp(event.target.value)} placeholder="08:47" className="min-h-11 rounded-xl bg-slate-950 px-3 text-sm"/><button type="button" onClick={addAnchor} className="min-h-11 rounded-xl bg-amber-400 px-4 text-xs font-black text-slate-950">ANCLAR</button></div></div>}
    <div className="mt-3 space-y-2">{segment.anchors.map((anchor) => { const event = session.events.find((item) => item.id === anchor.eventId); const resolution = event ? resolveEventVideoPosition(session, event.id) : null; return <div key={anchor.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-slate-900 px-3 py-2 text-xs"><span className="min-w-0 truncate"><strong>{formatVideoTimestamp(anchor.videoSecond)}</strong> · {event ? `P${event.period} min ${event.minute} · ${eventDescription(event, session.players)}` : "Evento no disponible"}</span><span className="flex gap-2">{resolution?.status === "RESOLVED" && <a href={resolution.url} target="_blank" rel="noopener noreferrer" className="inline-grid min-h-9 place-items-center rounded-lg bg-red-600 px-3 font-black">▶ PROBAR</a>}<button type="button" onClick={() => correctAnchor(anchor)} className="min-h-9 rounded-lg bg-slate-700 px-3 font-black">CORREGIR</button><button type="button" onClick={() => onCommit(removeVideoAnchor(session, segment.id, anchor.id))} className="min-h-9 rounded-lg bg-slate-700 px-3">×</button></span></div>; })}{segment.anchors.length === 0 && <p className="text-xs text-amber-300">Pendiente de ancla: el vídeo está guardado pero todavía no puede posicionar jugadas.</p>}</div>
  </section>;
}
