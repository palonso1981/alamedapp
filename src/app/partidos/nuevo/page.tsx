...
export default function NewMatchPage() {
  ...
  return <div className="min-h-screen bg-slate-900 text-white"><AppHeader title="Nuevo partido" /><main className="mx-auto max-w-2xl p-4 sm:p-8"><form onSubmit={submit} className="space-y-5 rounded-3xl border border-slate-700 bg-slate-800 p-5 sm:p-7">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-bold text-slate-400">Tipo<select name="competitionType" value={competitionType} onChange={(event) => setCompetitionType(event.target.value as CompetitionType)} className="mt-1 min-h-11 w-full rounded-xl bg-slate-950 px-3 text-white"><option value="LEAGUE">Liga</option><option value="CUP">Copa</option><option value="FRIENDLY">Amistoso</option><option value="OTHER">Otros</option></select></label>{competitionType === "OTHER" && <label className="text-xs font-bold text-slate-400">Detalle<input name="competitionOtherDetail" placeholder="Torneo, triangular…" className="mt-1 min-h-11 w-full rounded-xl bg-slate-950 px-3 text-white" /></label>}<label className="text-xs font-bold text-slate-400">Competición<input name="competition" placeholder="Preferente Futsal" className="mt-1 min-h-11 w-full rounded-xl bg-slate-950 px-3 text-white" /></label><label className="text-xs font-bold text-slate-400">Jornada<input name="matchday" type="number" min="1" step="1" inputMode="numeric" className="mt-1 min-h-11 w-full rounded-xl bg-slate-950 px-3 text-white" /></label><label className="text-xs font-bold text-slate-400 sm:col-span-2">Categoría<input name="category" className="mt-1 min-h-11 w-full rounded-xl bg-slate-950 px-3 text-white" /></label></div>
    {error && <p role="alert" className="rounded-xl bg-red-950 p-3 text-red-200">{error}</p>}<button type="submit" disabled={!seasonId} className="min-h-14 w-full rounded-xl bg-amber-400 text-lg font-black text-slate-950 disabled:opacity-30">CREAR Y PREPARAR →</button>
  </form></main></div>;
}
