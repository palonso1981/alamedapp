...
import { MAX_CALLED_PLAYERS, plannedMinutes, validatePreparation, validateStartingLineup } from "../../../../lib/preMatch";
...
export default function PreMatchPage() {
  ...
  return <div className="min-h-screen bg-slate-900 text-white"><AppHeader title={`Prepartido · ${preparation.opponent}`} actions={<SyncStatusBadge matchId={matchId} />} /><main className="mx-auto max-w-7xl space-y-5 p-3 sm:p-6">
    ...
    <section id="convocatoria" className="rounded-2xl border border-slate-700 bg-slate-800 p-4">
      <div className="flex items-center justify-between"><div><p className="text-xs font-black text-cyan-300">1 · CONVOCATORIA</p><h3 className="text-xl font-black">{preparation.calledPlayerIds.length} / {MAX_CALLED_PLAYERS}</h3></div><span className="text-sm text-slate-400">Toca para convocar</span></div>
