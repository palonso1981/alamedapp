...
import { GoalkeeperAnalysisColumn } from "../../../../components/dashboard/GoalkeeperAnalysisColumn";
...
export default function GoalkeeperDashboardPage({ params }: { params: { goalkeeperId: string } }) {
  ...
  return <div className="min-h-screen overflow-x-clip bg-slate-950 text-white"><div className="sticky top-0 z-40"><AppHeader title="Portero · Dashboard" clubId={data.fixture ? undefined : data.currentClubId}/></div><main className="mx-auto max-w-7xl space-y-5 p-3 pb-16 sm:p-5">
    <DashboardFilterBar clubName={data.workspace?.club.name ?? "Club"} clubs={data.clubs} onClub={data.setCurrentClub} scope={data.scope} referenceScope={data.referenceScope} teams={data.teams} seasons={data.seasons} matches={data.matches} rivals={data.rivals} players={data.analysis.players} goalkeepers={data.analysis.goalkeepers} referencePreset={data.referencePreset} mode={data.mode} onScope={data.setScope} onReferenceScope={data.setReferenceScope} onReferencePreset={data.setReferencePreset} onMode={data.setMode} onRefresh={data.refresh} fixture={data.fixture} comparison={<ComparisonHeader left={keeper?.name.toUpperCase() ?? "PORTERO"} right={comparison?.name.toUpperCase() ?? "SELECCIONA PORTERO B"} scope={data.scope}/>}/>
    ...
    {!keeper ? <p className="rounded-3xl border border-dashed border-slate-700 p-10 text-center text-slate-500">Portero sin minutos normales en este scope. Los intervalos P-J permanecen separados.</p> : <>
      ...
      {comparison && <section id="goalkeeper-comparison" className="grid gap-3 lg:grid-cols-2" aria-label="Comparación simétrica de porteros"><GoalkeeperAnalysisColumn keeper={keeper} records={data.analysis.records} trend={trend} onPoint={setSelectedPoint}/><GoalkeeperAnalysisColumn keeper={comparison} records={data.analysis.records} onPoint={setSelectedPoint}/></section>}
    </>}
  </main>{selectedPoint && <EventTracePanel records={data.analysis.records} point={selectedPoint} onClose={() => setSelectedPoint(null)} returnTo={`/dashboard/portero/${encodeURIComponent(goalkeeperId)}?${data.query}#goalkeeper-maps`}/>}</div>;
}
