      ...
      <main className="mx-auto max-w-7xl space-y-8 p-3 pb-16 sm:p-5 lg:p-7">
        ...
        {analytics.matches === 0 ? (
          ...
          {area === "RESUMEN" && <section>
            ...
            {matchId !== "ALL" && <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><ComparisonMetric label="AMENAZAS CDA" comparison={comparisons.threatsFor} referenceLabel={referenceLabel} /><ComparisonMetric label="AMENAZAS RIV" comparison={comparisons.threatsAgainst} referenceLabel={referenceLabel} /><ComparisonMetric label="GF" comparison={comparisons.goalsFor} referenceLabel={referenceLabel} /><ComparisonMetric label="GC" comparison={comparisons.goalsAgainst} referenceLabel={referenceLabel} /></div>}
            <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Trend label="AMENAZAS CDA" values={analysis.trends.map((point) => point.threatsFor)} reference={reference.samples ? reference.analytics.threats.FOR.total / reference.samples : null} /><Trend label="AMENAZAS RIV" values={analysis.trends.map((point) => point.threatsAgainst)} reference={reference.samples ? reference.analytics.threats.AGAINST.total / reference.samples : null} /><Trend label="GF" values={analysis.trends.map((point) => point.goalsFor)} reference={reference.samples ? reference.analytics.goalsFor / reference.samples : null} /><Trend label="GC" values={analysis.trends.map((point) => point.goalsAgainst)} reference={reference.samples ? reference.analytics.goalsAgainst / reference.samples : null} /></div>
          </section>}
          ...
          {area === "JUGADORES" && <section>
            ...
            <div className="mb-3 grid gap-2 sm:grid-cols-3">
              ...
              <select aria-label="Ordenar jugadores" value={sortPlayer} onChange={(event) => setSortPlayer(event.target.value as typeof sortPlayer)} className="min-h-11 rounded-xl bg-slate-800 px-3 text-xs font-black"><option value="MINUTES">ORDEN · MINUTOS</option><option value="GOALS">ORDEN · GOLES</option><option value="THREATS">ORDEN · AMENAZAS</option><option value="PLUS_MINUS">ORDEN · +/-</option></select>
            </div>
            <div className="overflow-x-auto rounded-3xl border border-slate-700 bg-slate-900">
              <div className="min-w-[720px]">
                <div className="grid grid-cols-[4rem_minmax(11rem,1fr)_repeat(7,minmax(4rem,auto))] gap-2 border-b border-slate-700 px-3 py-2 text-[9px] font-black text-slate-500"><span>#</span><span>JUGADOR</span>{playerView === "GENERAL" && <><span>PJ</span><span>MIN</span><span>%</span><span>G</span><span>A</span><span>AMEN.</span><span>OBJ/±</span></>}{playerView === "EN_PISTA" && <><span>FOR</span><span>AGAINST</span><span>GF</span><span>GC</span><span>PARCIAL</span><span>+/-</span><span>MIN</span></>}{playerView === "POR_40" && <><span>FOR/40</span><span>AG/40</span><span>Δ AM.</span><span>GF/40</span><span>GC/40</span><span>Δ GOL</span><span>MIN</span></>}{playerView === "DISCIPLINA" && <><span>FC</span><span>FR</span><span>FC CRÍT.</span><span>FR CRÍT.</span><span>TA</span><span>TR</span><span>MIN</span></>}</div>
          ...
          {area === "EQUIPO" && <section>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6"><Metric label="AMEN. CDA" value={analytics.threats.FOR.total} /><Metric label="AMEN. CDA /40" value={number(analysis.rates.threatsFor40)} /><Metric label="AMEN. RIV" value={analytics.threats.AGAINST.total} /><Metric label="AMEN. RIV /40" value={number(analysis.rates.threatsAgainst40)} /><Metric label="GF · /40" value={`${analytics.goalsFor} · ${number(analysis.rates.goalsFor40)}`} /><Metric label="GC · /40" value={`${analytics.goalsAgainst} · ${number(analysis.rates.goalsAgainst40)}`} /><Metric label="FC · /40" value={`${analytics.discipline.for.fouls} · ${number(analysis.rates.foulsFor40)}`} /><Metric label="FR · /40" value={`${analytics.discipline.against.fouls} · ${number(analysis.rates.foulsAgainst40)}`} /><Metric label="FC CRÍTICAS" value={analysis.criticalFouls.for} /><Metric label="FR CRÍTICAS" value={analysis.criticalFouls.against} /></div>
          </section>}
