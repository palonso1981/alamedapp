  ...
  return <div className="min-h-screen overflow-x-hidden bg-slate-950 text-white">
    ...
    <main className="mx-auto max-w-6xl space-y-5 p-3 pb-16 sm:p-6">
      ...
      {!player ? <section className="rounded-3xl border border-dashed border-slate-700 p-10 text-center"><strong>Jugador sin datos en este alcance</strong><p className="mt-2 text-sm text-slate-500">N/D · revisa equipo, temporada o partido.</p></section> : <>
        ...
        <section><h2 className="mb-3 font-black">ACCIONES PROPIAS</h2><div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7"><Kpi label="MINUTOS" value={number(player.minutes)} /><Kpi label="G" value={player.goals} /><Kpi label="ASIST. CONFIRMADAS" value={player.assists} /><Kpi label="REMATES" value={player.ownThreats} /><Kpi label="REMATES /40" value={number(player.ownThreats40)} /><Kpi label="FALTAS C/R" value={`${player.foulsCommitted}/${player.foulsReceived}`} /><Kpi label="CRÍTICAS C/R" value={`${player.criticalFoulsCommitted}/${player.criticalFoulsReceived}`} /></div></section>
        <section><h2 className="mb-3 font-black">EQUIPO CON ÉL EN PISTA</h2><div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7"><Kpi label="REMATES" value={player.onCourt.threatsFor} /><Kpi label="AMENAZAS" value={player.onCourt.threatsAgainst} /><Kpi label="REMATES /40" value={number(player.onCourt.threatsFor40)} /><Kpi label="AMENAZAS /40" value={number(player.onCourt.threatsAgainst40)} /><Kpi label="PARCIAL" value={`${player.onCourt.goalsFor}–${player.onCourt.goalsAgainst}`} /><Kpi label="+/-" value={player.onCourt.goalDifference > 0 ? `+${player.onCourt.goalDifference}` : player.onCourt.goalDifference} /><Kpi label="GF/40 · GC/40" value={`${number(player.onCourt.goalsFor40)} · ${number(player.onCourt.goalsAgainst40)}`} /></div><p className="mt-2 text-[10px] text-slate-500">Describe el comportamiento colectivo durante sus intervalos, sin afirmar causalidad.</p></section>
        <div className="grid gap-4 lg:grid-cols-2"><MinuteTrend values={player.trend} /><div><div className="mb-2 flex flex-wrap gap-2">{(["ALL", "GOL", "PARADA", "FUERA"] as const).map((item) => <button key={item} type="button" onClick={() => setShotFilter(item)} className={`min-h-10 rounded-xl px-3 text-xs font-black ${shotFilter === item ? "bg-cyan-300 text-slate-950" : "bg-slate-800"}`}>{item === "ALL" ? "TODOS LOS REMATES" : item}</button>)}</div><PitchThreatMap points={points} side="FOR" /></div></div>
