...
export function PlayerTableV2({ players, scores, mode, detailQuery }: { players: PlayerAnalysis[]; scores: PlayerScore[]; mode: DashboardValueMode; detailQuery: string }) {
  ...
  const metric = (player: PlayerAnalysis): number | null => {
    ...
    if (column === "threatBalance") return player.onCourt.threatsFor - player.onCourt.threatsAgainst;
    if (column === "onTargetFor") return player.onCourt.threatsForOnTarget;
    if (column === "onTargetAgainst") return player.onCourt.threatsAgainstOnTarget;
    if (column === "onTargetBalance") return player.onCourt.threatsForOnTarget - player.onCourt.threatsAgainstOnTarget;
    if (column === "nearFor") return player.onCourt.threatsForNear;
    if (column === "nearAgainst") return player.onCourt.threatsAgainstNear;
    if (column === "nearBalance") return player.onCourt.threatsForNear - player.onCourt.threatsAgainstNear;
