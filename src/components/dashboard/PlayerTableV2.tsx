...
type Column = "minutes" | "goals" | "assists" | "threats" | "points" | "plusMinus" | "threatsFor" | "threatsAgainst" | "foulsCommitted" | "foulsReceived" | "criticalCommitted" | "criticalReceived" | "score";
...
export function PlayerTableV2({ players, scores, mode, detailQuery }: { players: PlayerAnalysis[]; scores: PlayerScore[]; mode: DashboardValueMode; detailQuery: string }) {
  ...
  const shown = view === "GENERAL"
    ? (["minutes", "goals", "assists", "threats", "points", "score"] as Column[])
    : view === "ON_COURT"
      ? (["minutes", "threatsFor", "threatsAgainst", "plusMinus", "points"] as Column[])
      : (["minutes", "foulsCommitted", "foulsReceived", "criticalCommitted", "criticalReceived"] as Column[]);
  const labels: Record<Column, string> = { minutes: "MIN", goals: "G", assists: "ASIST.", threats: "REM.", points: "PTS PISTA", plusMinus: "+/-", threatsFor: "REM. EQ.", threatsAgainst: "AMEN.", foulsCommitted: "FC", foulsReceived: "FR", criticalCommitted: "FC 5+", criticalReceived: "FR 5+", score: "SCORE" };
  return <section>
    ...
    <div className="overflow-x-auto rounded-3xl border border-slate-700 bg-slate-900">
      <div className="min-w-[760px]">
        <div className="grid grid-cols-[4rem_minmax(12rem,1fr)_repeat(6,minmax(5rem,auto))] items-center gap-3 border-b border-slate-700 px-4 py-1"><span className="text-[9px] font-black text-slate-500">#</span><span className="text-[9px] font-black text-slate-500">JUGADOR</span>{shown.map((id) => <Header key={id} id={id} label={labels[id]} column={column} direction={direction} onSort={sort} />)}</div>
        {sorted.map((player) => <Link key={player.playerId} href={`/dashboard/jugador/${encodeURIComponent(player.playerId)}?${detailQuery}`} className="grid min-h-16 grid-cols-[4rem_minmax(12rem,1fr)_repeat(6,minmax(5rem,auto))] items-center gap-3 border-b border-slate-800 px-4 py-2 last:border-0 hover:bg-slate-800/70">
          <strong className="text-cyan-300">#{player.number}</strong><span className="min-w-0"><strong className="block truncate">{player.name}</strong><small className="text-[9px] text-slate-500">{player.position ?? "N/D"} · {player.matches} PJ{player.lowSample ? " · MUESTRA BAJA" : ""}</small></span>
