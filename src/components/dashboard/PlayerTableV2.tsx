...
import { DashboardAnalysis, PlayerAnalysis } from "../../lib/dashboardAnalysis";
import { DashboardValueMode, PlayerScore, playerMetricValue, SortDirection, stableSortByMetric, teamPlayerTableMetricValue, TeamPlayerTableMetricKey } from "../../lib/dashboardV2";
...
type Column = TeamPlayerTableMetricKey;
...
export function PlayerTableV2({ analysis, players, scores, mode, detailQuery }: { analysis: DashboardAnalysis; players: PlayerAnalysis[]; scores: PlayerScore[]; mode: DashboardValueMode; detailQuery: string }) {
    ...
    : view === "ON_COURT" ? (["minutes", "goalsFor", "goalsAgainst", "plusMinus", "threatsFor", "threatsAgainst", "threatBalance", "points"] as Column[])
      : view === "DANGER" ? (["minutes", "onTargetFor", "onTargetAgainst", "onTargetBalance", "nearFor", "nearAgainst", "nearBalance"] as Column[])
        : view === "CONTEXT" ? (["minutes", "keyMinutes", "keyPercentage", "goldMinutes", "goldPercentage", "points"] as Column[])
          : (["minutes", "foulsCommitted", "foulsReceived", "criticalCommitted", "criticalReceived", "yellowCards", "redCards"] as Column[]);
  const labels: Record<Column, string> = { matches: "PJ", minutes: "MIN", avgMinutes: "MIN/PART.", goals: "G", assists: "ASIST.", threats: "REM.", possessionLosses: "PÉRD.", points: "PTS", plusMinus: "DIF. GOLES", goalsFor: "GF", goalsAgainst: "GC", threatsFor: "REM. EQ.", threatsAgainst: "AMEN.", threatBalance: "BAL. REM/AME", onTargetFor: "REM. PUERTA", onTargetAgainst: "AME. PUERTA", onTargetBalance: "BAL. PUERTA", nearFor: "REM. CERCA", nearAgainst: "AME. CERCA", nearBalance: "BAL. CERCA", keyMinutes: "MIN. CLAVE", keyPercentage: "% CLAVE", goldMinutes: "MIN. ORO", goldPercentage: "% ORO", foulsCommitted: "FC", foulsReceived: "FR", criticalCommitted: "FC 5+", criticalReceived: "FR 5+", yellowCards: "TA", redCards: "TR", score: "SCORE" };
  const teamValues = new Map(shown.map((id) => [id, teamPlayerTableMetricValue(analysis, id, mode)]));
  return <section>
    ...
    <div className="overflow-x-auto rounded-3xl border border-slate-700 bg-slate-900">
      <div style={{ minWidth: `${300 + shown.length * 82}px` }}>
        ...
        <div data-team-summary className="grid min-h-14 items-center gap-3 border-b border-cyan-800/60 bg-cyan-950/30 px-4 py-2" style={{ gridTemplateColumns: `minmax(15rem,1fr) repeat(${shown.length},minmax(4.5rem,auto))` }} title="Agregado real del equipo dentro del contexto y filtros seleccionados."><span><strong className="block text-xs text-cyan-200">EQUIPO</strong><small className="text-[9px] text-slate-400">{analysis.analytics.matches} partidos · {format(analysis.rates.observedMinutes)} min</small></span>{shown.map((id) => <span key={id} className="text-right text-sm font-black text-cyan-100">{format(teamValues.get(id) ?? null)}</span>)}</div>
...
function metricForColumn(player: PlayerAnalysis, column: Column, mode: DashboardValueMode): number | null {
  ...
  if (column === "yellowCards") return normalize(player.yellowCards);
  if (column === "redCards") return normalize(player.redCards);
