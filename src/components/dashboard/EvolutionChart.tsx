...
export function EvolutionChart({ points, reference, referenceLabel }: { points: EvolutionPoint[]; reference: number | null; referenceLabel: string }) {
  ...
  return <div ref={host} data-evolution-chart data-point-count={points.length} className="w-full min-w-0 overflow-hidden rounded-3xl border border-slate-700 bg-slate-900 p-3 sm:p-4"><div className="relative h-56 w-full min-w-0">
    ...
    <div className="absolute inset-0 grid items-end border-b border-slate-700 px-1 pb-7" style={{ gridTemplateColumns: `repeat(${Math.max(1, points.length)}, minmax(0, 1fr))`, gap: `${layout.gap}px` }}>{points.map((point, index) => <button type="button" key={point.id} title={`${point.label} · ${point.detail} · ${point.value === null ? "N/D" : format(point.value)} · ${referenceLabel} ${reference === null ? "N/D" : format(reference)}`} className="group relative h-full min-w-0"><span className="absolute inset-x-0 bottom-0 min-w-px rounded-t bg-cyan-400 transition group-focus:bg-cyan-200" style={{ height: point.value === null ? "0" : `${Math.max(3, point.value / max * 82)}%` }}/>{(index % layout.labelEvery === 0 || index === points.length - 1) && <strong className="absolute inset-x-0 bottom-[-22px] truncate text-[8px] text-slate-400">{point.label}</strong>}{point.value !== null && points.length <= 15 && <span className="absolute inset-x-0 text-center text-[8px] font-black text-white" style={{ bottom: `${Math.max(4, point.value / max * 82 + 1)}%` }}>{format(point.value)}</span>}</button>)}</div>
  </div></div>;
}
