...
export function scopeLabel(scope: DashboardScopeV2): string {
  ...
  const parts = [competition, scope.venues.length === 1 ? scope.venues[0] === "HOME" ? "Local" : "Visitante" : null, scope.period === "ALL" ? null : `P${scope.period}`, scope.scoreState === "LEADING" ? "Ganando" : scope.scoreState === "DRAWING" ? "Empatando" : scope.scoreState === "TRAILING" ? "Perdiendo" : null, scope.playingState === "PJ_CDA" ? "PJ CDA" : scope.playingState === "PJ_RIVAL" ? "PJ rival" : null, scope.competitiveContext === "KEY" ? "Minutos clave" : scope.competitiveContext === "GOLD" ? "Minutos de oro" : null].filter(Boolean);
