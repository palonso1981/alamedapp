...
function Summary({ analysis, reference, mode, query, comparisonEnabled }: { analysis: Analysis; reference: Analysis; mode: DashboardValueMode; query: string; comparisonEnabled: boolean }) {
  ...
  return <>
    ...
    <section className="grid gap-3 lg:grid-cols-2"><OutcomeDistribution side="FOR" stats={analysis.analytics.threats.FOR} reference={comparisonEnabled ? reference.analytics.threats.FOR : undefined} /><OutcomeDistribution side="AGAINST" stats={analysis.analytics.threats.AGAINST} reference={comparisonEnabled ? reference.analytics.threats.AGAINST : undefined} /></section>
    ...
    <TeamEvolution analysis={analysis} reference={comparisonEnabled ? reference : undefined} referenceLabel="MEDIA"/>
  </>;
}
...
function TeamArea({ analysis, reference, mode, scope, onScope, referenceLabel, comparisonEnabled }: { analysis: Analysis; reference: Analysis; mode: DashboardValueMode; scope: DashboardScopeV2; onScope: (scope: DashboardScopeV2) => void; referenceLabel: string; comparisonEnabled: boolean }) {
  ...
  return <><section><SectionTitle eyebrow="SIN AGREGADOS PERSISTIDOS">EQUIPO</SectionTitle><TeamComparison analysis={analysis} reference={reference} mode={mode} comparisonEnabled={comparisonEnabled} /><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5"><Kpi label="PÉRDIDAS" value={format(teamMetricValue(analysis, "possessionLosses", mode))} help="POSSESSION_LOSSES"/><Kpi label="FC CRÍTICAS" value={analysis.criticalFouls.for} /><Kpi label="FR CRÍTICAS" value={analysis.criticalFouls.against} /><Kpi label="2ª JUGADA" value={analysis.analytics.secondPlay.threats} /><Kpi label="GOLES 2ª" value={analysis.analytics.secondPlay.goals} /></div></section><section><SectionTitle eyebrow="REMATES ← CENTRO → AMENAZAS">FASES</SectionTitle><div className="space-y-3 rounded-3xl border border-slate-700 bg-slate-900 p-4">{DASHBOARD_PHASES.filter((phase) => phase !== "UNSPECIFIED").map((phase) => { const current = analysis.analytics.phases[phase]; return <button type="button" key={phase} onClick={() => onScope({ ...scope, phases: toggle(scope.phases, phase) })} className="block w-full"><FacedMetricRow label={PHASE_LABEL[phase]} own={current.FOR} rival={current.AGAINST} ownReference={comparisonEnabled && reference.samples ? reference.analytics.phases[phase].FOR / reference.samples : null} rivalReference={comparisonEnabled && reference.samples ? reference.analytics.phases[phase].AGAINST / reference.samples : null} ownGoals={phaseGoals(phase, "FOR")} rivalGoals={phaseGoals(phase, "AGAINST")} showReference={comparisonEnabled} /></button>; })}</div></section><TeamEvolution analysis={analysis} reference={comparisonEnabled ? reference : undefined} referenceLabel={referenceLabel}/><section><SectionTitle eyebrow="ESTADO SOSTENIDO · NO FASE AISLADA">PORTERO-JUGADOR</SectionTitle><div className="grid gap-3 sm:grid-cols-2"><PlayingStateCard title="PJ CDA" tone="cyan" stats={analysis.flyingGoalkeeper.for}/><PlayingStateCard title="PJ RIVAL" tone="rose" stats={analysis.flyingGoalkeeper.against}/></div></section></>;
}
