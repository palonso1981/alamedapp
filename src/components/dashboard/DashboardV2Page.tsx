...
import { buildVideoLibraryItems, EMPTY_VIDEO_LIBRARY_FILTERS } from "../../lib/videoLibrary";
...
type DashboardFamily = "context" | "attack" | "defense" | "setPiece" | "discipline";

const FAMILY_STYLE: Record<DashboardFamily, { icon: string; label: string; frame: string; accent: string; card: string }> = {
  context: { icon: "◫", label: "CONTEXTO / RESULTADO", frame: "border-indigo-800/80 bg-indigo-950/20", accent: "text-indigo-200", card: "border-l-indigo-400" },
  attack: { icon: "↗", label: "ATAQUE", frame: "border-cyan-800/80 bg-cyan-950/20", accent: "text-cyan-200", card: "border-l-cyan-400" },
  defense: { icon: "◆", label: "DEFENSA", frame: "border-violet-800/80 bg-violet-950/20", accent: "text-violet-200", card: "border-l-violet-400" },
  setPiece: { icon: "⌟", label: "BALÓN PARADO", frame: "border-amber-800/80 bg-amber-950/20", accent: "text-amber-200", card: "border-l-amber-400" },
  discipline: { icon: "▮", label: "DISCIPLINA", frame: "border-fuchsia-800/80 bg-fuchsia-950/20", accent: "text-fuchsia-200", card: "border-l-fuchsia-400" },
};

function FamilySection({ family, title, eyebrow, children }: { family: DashboardFamily; title?: string; eyebrow?: string; children: React.ReactNode }) {
  const style = FAMILY_STYLE[family];
  return <section data-dashboard-family={family} className={`rounded-3xl border p-3 sm:p-4 ${style.frame}`}>
    <div className="mb-4 flex items-center gap-3">
      <span aria-hidden="true" className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl border border-current/30 bg-slate-950/70 text-xl ${style.accent}`}>{style.icon}</span>
      <div>{eyebrow && <p className={`text-[9px] font-black tracking-[.16em] ${style.accent}`}>{eyebrow}</p>}<h2 className="text-lg font-black sm:text-xl">{title ?? style.label}</h2></div>
    </div>
    {children}
  </section>;
}

function Kpi({ label, value, detail, tone = "text-white", help, family, detailProminent = false }: { label: string; value: string | number; detail?: string; tone?: string; help?: React.ComponentProps<typeof MetricHelp>["metricId"]; family?: DashboardFamily; detailProminent?: boolean }) {
  const familyStyle = family ? FAMILY_STYLE[family] : null;
  return <article className={`rounded-2xl border border-slate-700 bg-slate-900/90 p-3 ${familyStyle ? `border-l-4 ${familyStyle.card}` : ""}`}><div className="flex items-center justify-between gap-2"><p className={`text-[9px] font-black tracking-[.12em] ${familyStyle?.accent ?? "text-slate-500"}`}>{label}</p>{help && <MetricHelp metricId={help}/>}</div><strong className={`mt-1 block text-2xl sm:text-3xl ${tone}`}>{value}</strong>{detail && <span data-kpi-detail={detailProminent ? "prominent" : "secondary"} className={`mt-1 block ${detailProminent ? `text-base font-black sm:text-lg ${familyStyle?.accent ?? "text-cyan-200"}` : "text-[9px] text-slate-500"}`}>{detail}</span>}</article>;
}
...
function TeamEvolution({ analysis, reference, referenceLabel }: { analysis: Analysis; reference?: Analysis; referenceLabel: string }) {
  ...
  return <div><div className="mb-3 flex justify-end"><select aria-label="Métrica de evolución" value={metric} onChange={(event) => setMetric(event.target.value as TeamEvolutionMetric)} className="min-h-11 rounded-xl bg-slate-800 px-3 text-xs font-black">{TEAM_EVOLUTION.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></div><EvolutionChart points={analysis.trends.map((item) => ({ id: item.matchId, label: item.opponent, detail: `${item.date} · ${item.venue === "HOME" ? "Local" : "Visitante"} · ${item.goalsFor}-${item.goalsAgainst}`, value: trendValue(item, metric) }))} reference={referenceValue} referenceLabel={referenceLabel}/></div>;
}
...
export function DashboardV2Page() {
  ...
  }, [area, comparisonEnabled, mode, ready, referencePreset, referenceScope, router, scope, searchParams]);
  ...
  return <div className="min-h-screen overflow-x-clip bg-slate-950 text-white"><div className="sticky top-0 z-40"><AppHeader title="Dashboard V2" clubId={fixture ? undefined : currentClubId} actions={<><Link href={`/dashboard/jugadas?${query}`} className="grid min-h-10 place-items-center rounded-lg bg-red-950 px-3 text-[10px] font-black text-red-300">▶ JUGADAS</Link>{videoCount > 0 && <Link href={`/video?from=dashboard&${query}`} className="grid min-h-10 place-items-center rounded-lg bg-violet-950 px-3 text-[10px] font-black text-violet-200">🎬 VER VÍDEOS ({videoCount})</Link>}</>} /></div><main className="mx-auto max-w-7xl space-y-6 p-3 pb-16 sm:p-5">
    <DashboardFilterBar clubName={workspace?.club.name ?? "Club"} clubs={clubs} onClub={setCurrentClub} scope={scope} referenceScope={referenceScope} teams={teams} seasons={seasons} matches={matches} rivals={rivals} players={analysis.players} goalkeepers={analysis.goalkeepers} referencePreset={referencePreset} mode={mode} onScope={setScope} onReferenceScope={setReferenceScope} onReferencePreset={changeReferencePreset} onMode={setMode} comparisonEnabled={comparisonEnabled} onComparisonEnabled={setComparisonEnabled} onRefresh={() => setRecords(fixture ? buildDashboardFixture() : readLocalRecords())} fixture={fixture} comparison={<ComparisonHeader left={area === "GOALKEEPERS" ? keeperA?.name.toUpperCase() ?? "PORTERO A" : "CDA"} right={area === "GOALKEEPERS" ? keeperB?.name.toUpperCase() ?? "PORTERO B" : comparisonRightLabel(referenceScope, referencePreset, matches.find((match) => match.matchId === referenceScope.matchIds[0])?.opponent)} scope={scope}/>} />
    ...
    {!ready ? <p className="p-10 text-center text-slate-500">Preparando análisis…</p> : analysis.analytics.matches === 0 ? <Empty /> : <>{area === "SUMMARY" && <Summary analysis={analysis} reference={reference} mode={mode} query={query} comparisonEnabled={comparisonEnabled} />}{area === "TEAM" && <TeamArea analysis={analysis} reference={reference} mode={mode} scope={scope} onScope={setScope} referenceLabel={comparisonRightLabel(scope, referencePreset)} comparisonEnabled={comparisonEnabled} />}{area === "PLAYERS" && <><SectionTitle eyebrow="CABECERAS CUANTITATIVAS ORDENABLES">JUGADORES</SectionTitle><PlayerTableV2 analysis={analysis} players={analysis.players} scores={scores} mode={mode} detailQuery={query} /></>}{area === "GOALKEEPERS" && <Goalkeepers analysis={analysis} keeperA={keeperA} keeperB={keeperB} onA={setKeeperAId} onB={setKeeperBId} query={query} onPoint={setSelectedPoint} />}{area === "MAPS" && <Maps analysis={analysis} scope={scope} onScope={setScope} onPoint={setSelectedPoint} />}</>}
...
function Summary({ analysis, reference, mode, query, comparisonEnabled }: { analysis: Analysis; reference: Analysis; mode: DashboardValueMode; query: string; comparisonEnabled: boolean }) {
  ...
  const goalsFor = teamMetricValue(analysis, "goalsFor", mode);
  const goalsAgainst = teamMetricValue(analysis, "goalsAgainst", mode);
  const goalDifference = goalsFor === null || goalsAgainst === null ? null : goalsFor - goalsAgainst;
  const ownConversion = analysis.analytics.threats.FOR.total ? analysis.analytics.threats.FOR.GOL / analysis.analytics.threats.FOR.total * 100 : null;
  const rivalConversion = analysis.analytics.threats.AGAINST.total ? analysis.analytics.threats.AGAINST.GOL / analysis.analytics.threats.AGAINST.total * 100 : null;
  const savePercentage = interiors ? saves / interiors * 100 : null;
  return <>
    <FamilySection family="context" eyebrow="QUÉ OCURRIÓ"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5"><Kpi family="context" label="PARTIDOS" value={analysis.samples}/><Kpi family="context" label="MINUTOS OBS." value={format(teamPlayerTableMetricValue(analysis, "minutes", mode), "'")}/><Kpi family="context" label="GF" value={format(goalsFor)} tone="text-cyan-100"/><Kpi family="context" label="GC" value={format(goalsAgainst)} tone="text-violet-100"/><Kpi family="context" label="DIFERENCIA" value={format(goalDifference)} tone={goalDifference !== null && goalDifference > 0 ? "text-cyan-100" : "text-white"}/></div></FamilySection>
    {pending > 0 && <aside data-dashboard-quality-alert className="flex items-center gap-3 rounded-2xl border border-amber-800/70 bg-amber-950/30 px-4 py-3 text-xs text-amber-100"><strong className="text-lg">?</strong><span><b>{pending} pendientes de revisión</b> · siguen computando en el análisis.</span></aside>}
    <FamilySection family="attack" eyebrow="CUÁNTO GENERAMOS"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6"><Kpi family="attack" label="REMATES" value={format(teamMetricValue(analysis, "threatsFor", mode))} detail={`CONVERSIÓN ${format(ownConversion, "%")}`} detailProminent/><Kpi family="attack" label="GOLES" value={format(goalsFor)}/><Kpi family="attack" label="ASISTENCIAS" value={format(teamPlayerTableMetricValue(analysis, "assists", mode))}/><Kpi family="attack" label="A PUERTA" value={ownDerived.onTarget} detail={format(ownDerived.onTargetPercentage, "%")} detailProminent help="ON_TARGET"/><Kpi family="attack" label="CERCANOS" value={ownDerived.near} detail={format(ownDerived.nearPercentage, "%")} detailProminent help="NEAR_ZONE"/><Kpi family="attack" label="PÉRDIDAS" value={format(teamMetricValue(analysis, "possessionLosses", mode))} help="POSSESSION_LOSSES"/></div><div className="mt-3"><OutcomeDistribution side="FOR" stats={analysis.analytics.threats.FOR} reference={comparisonEnabled ? reference.analytics.threats.FOR : undefined}/></div></FamilySection>
    <FamilySection family="defense" eyebrow="CUÁNTO CONCEDIMOS"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5"><Kpi family="defense" label="AMENAZAS" value={format(teamMetricValue(analysis, "threatsAgainst", mode))} detail={`GOL ${format(rivalConversion, "%")}`} detailProminent/><Kpi family="defense" label="GC" value={format(goalsAgainst)}/><Kpi family="defense" label="PARADAS" value={saves} detail={format(savePercentage, "%")} detailProminent help="SAVE_PERCENTAGE"/><Kpi family="defense" label="A PUERTA" value={againstDerived.onTarget} detail={format(againstDerived.onTargetPercentage, "%")} detailProminent help="ON_TARGET"/><Kpi family="defense" label="CERCANAS" value={againstDerived.near} detail={format(againstDerived.nearPercentage, "%")} detailProminent help="NEAR_ZONE"/></div><div className="mt-3"><OutcomeDistribution side="AGAINST" stats={analysis.analytics.threats.AGAINST} reference={comparisonEnabled ? reference.analytics.threats.AGAINST : undefined}/></div></FamilySection>
    <FamilySection family="setPiece"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3"><Kpi family="setPiece" label="CÓRNERS · CDA/RIV" value={`${analysis.analytics.phases.SET_PIECE_CORNER.FOR} / ${analysis.analytics.phases.SET_PIECE_CORNER.AGAINST}`}/><Kpi family="setPiece" label="BANDAS · CDA/RIV" value={`${analysis.analytics.phases.SET_PIECE_KICK_IN.FOR} / ${analysis.analytics.phases.SET_PIECE_KICK_IN.AGAINST}`}/><Kpi family="setPiece" label="FALTAS ABP · CDA/RIV" value={`${analysis.analytics.phases.SET_PIECE_FREE_KICK.FOR} / ${analysis.analytics.phases.SET_PIECE_FREE_KICK.AGAINST}`}/></div></FamilySection>
    <FamilySection family="discipline"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3"><Kpi family="discipline" label="FALTAS · CDA/RIV" value={`${analysis.analytics.discipline.for.fouls} / ${analysis.analytics.discipline.against.fouls}`}/><Kpi family="discipline" label="AMARILLAS · CDA/RIV" value={`${analysis.analytics.discipline.for.yellowCards} / ${analysis.analytics.discipline.against.yellowCards}`}/><Kpi family="discipline" label="ROJAS · CDA/RIV" value={`${analysis.analytics.discipline.for.redCards} / ${analysis.analytics.discipline.against.redCards}`}/></div></FamilySection>
    {comparisonEnabled && <section><SectionTitle eyebrow="ACTUAL SÓLIDO · REFERENCIA DISCONTINUA">COMPARACIÓN</SectionTitle><TeamComparison analysis={analysis} reference={reference} mode={mode} /></section>}
    ...
    <FamilySection family="context" title="EVOLUCIÓN" eyebrow="CONTEXTO TEMPORAL"><TeamEvolution analysis={analysis} reference={comparisonEnabled ? reference : undefined} referenceLabel="MEDIA"/></FamilySection>
  </>;
}

function TeamArea({ analysis, reference, mode, scope, onScope, referenceLabel, comparisonEnabled }: { analysis: Analysis; reference: Analysis; mode: DashboardValueMode; scope: DashboardScopeV2; onScope: (scope: DashboardScopeV2) => void; referenceLabel: string; comparisonEnabled: boolean }) {
  ...
  return <>
    <FamilySection family="context" title="BALANCES ATAQUE / DEFENSA" eyebrow="EQUIPO · AGREGADO DEL SCOPE"><TeamComparison analysis={analysis} reference={reference} mode={mode} comparisonEnabled={comparisonEnabled}/></FamilySection>
    <FamilySection family="context" title="FASES" eyebrow="REMATES ← CENTRO → AMENAZAS"><div className="space-y-3 rounded-2xl border border-slate-700 bg-slate-900 p-3 sm:p-4">{DASHBOARD_PHASES.filter((phase) => phase !== "UNSPECIFIED").map((phase) => { const current = analysis.analytics.phases[phase]; return <button type="button" key={phase} onClick={() => onScope({ ...scope, phases: toggle(scope.phases, phase) })} className="block w-full"><FacedMetricRow label={PHASE_LABEL[phase]} own={current.FOR} rival={current.AGAINST} ownReference={comparisonEnabled && reference.samples ? reference.analytics.phases[phase].FOR / reference.samples : null} rivalReference={comparisonEnabled && reference.samples ? reference.analytics.phases[phase].AGAINST / reference.samples : null} ownGoals={phaseGoals(phase, "FOR")} rivalGoals={phaseGoals(phase, "AGAINST")} showReference={comparisonEnabled}/></button>; })}</div></FamilySection>
    <FamilySection family="attack" title="SEGUNDA JUGADA" eyebrow="CONTINUIDAD OFENSIVA"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3"><Kpi family="attack" label="PÉRDIDAS" value={format(teamMetricValue(analysis, "possessionLosses", mode))} help="POSSESSION_LOSSES"/><Kpi family="attack" label="2ª JUGADA" value={analysis.analytics.secondPlay.threats}/><Kpi family="attack" label="GOLES 2ª" value={analysis.analytics.secondPlay.goals}/></div></FamilySection>
    <FamilySection family="discipline" title="DISCIPLINA CONTEXTUAL" eyebrow="SITUACIONES CRÍTICAS"><div className="grid grid-cols-2 gap-2"><Kpi family="discipline" label="FC CRÍTICAS" value={analysis.criticalFouls.for}/><Kpi family="discipline" label="FR CRÍTICAS" value={analysis.criticalFouls.against}/></div></FamilySection>
    <FamilySection family="context" title="PORTERO-JUGADOR" eyebrow="ESTADO SOSTENIDO · NO FASE AISLADA"><div className="grid gap-3 sm:grid-cols-2"><PlayingStateCard title="PJ CDA" tone="cyan" stats={analysis.flyingGoalkeeper.for}/><PlayingStateCard title="PJ RIVAL" tone="rose" stats={analysis.flyingGoalkeeper.against}/></div></FamilySection>
    <FamilySection family="context" title="EVOLUCIÓN" eyebrow="CONTEXTO TEMPORAL"><TeamEvolution analysis={analysis} reference={comparisonEnabled ? reference : undefined} referenceLabel={referenceLabel}/></FamilySection>
  </>;
}
