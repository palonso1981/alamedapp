...
import { buildDashboardV2, buildPlayerScores, DashboardArea, DashboardReferencePreset, DashboardScopeV2, DashboardValueMode, emptyDashboardScope, mergeDashboardSearchParams, referenceScopeForPreset, scopeFromSearchParams, teamMetricValue } from "../../lib/dashboardV2";
...
function Kpi({ label, value, detail, tone = "text-white" }: { label: string; value: string | number; detail?: string; tone?: string }) {
  return <article className="rounded-2xl border border-slate-700 bg-slate-900 p-3"><p className="text-[9px] font-black tracking-[.12em] text-slate-500">{label}</p><strong className={`mt-1 block text-2xl sm:text-3xl ${tone}`}>{value}</strong>{detail && <span className="mt-1 block text-[9px] text-slate-500">{detail}</span>}</article>;
}
...
export function DashboardV2Page() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-slate-950 text-white"><AppHeader title="Dashboard V2" clubId={fixture ? undefined : currentClubId} /><main className="mx-auto max-w-7xl space-y-6 p-3 pb-16 sm:p-5">
    ...
    {!ready ? <p className="p-10 text-center text-slate-500">Preparando análisis…</p> : analysis.analytics.matches === 0 ? <Empty /> : <>{area === "SUMMARY" && <Summary analysis={analysis} reference={reference} mode={mode} query={query} />}{area === "TEAM" && <TeamArea analysis={analysis} reference={reference} mode={mode} scope={scope} onScope={setScope} />}{area === "PLAYERS" && <><SectionTitle eyebrow="CABECERAS CUANTITATIVAS ORDENABLES">JUGADORES</SectionTitle><PlayerTableV2 players={analysis.players} scores={scores} mode={mode} detailQuery={query} /></>}{area === "GOALKEEPERS" && <Goalkeepers analysis={analysis} keeperA={keeperA} keeperB={keeperB} onA={setKeeperAId} onB={setKeeperBId} query={query} />}{area === "MAPS" && <Maps analysis={analysis} scope={scope} onScope={setScope} />}</>}
  </main></div>;
}
...
function Maps({ analysis, scope, onScope }: { analysis: Analysis; scope: DashboardScopeV2; onScope: (scope: DashboardScopeV2) => void }) {
  return <section><SectionTitle eyebrow="PUNTOS RAW + AGREGACIÓN FILTRABLE">MAPAS / ZONAS</SectionTitle><div className="grid gap-3 lg:grid-cols-2"><PitchThreatMap points={analysis.analytics.pitchPoints} side="FOR"/><PitchThreatMap points={analysis.analytics.pitchPoints} side="AGAINST"/><PitchZoneGrid zones={analysis.pitchZones} selected={scope.originZones} onSelect={(zone) => onScope({ ...scope, originZones: toggle(scope.originZones, zone) })}/><GoalThreatMap points={analysis.analytics.goalPoints}/><GoalZoneGrid zones={analysis.goalZones} selected={scope.targetZones} onSelect={(zone) => onScope({ ...scope, targetZones: toggle(scope.targetZones, zone) })}/></div></section>;
}
