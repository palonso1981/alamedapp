...
export function DashboardFilterBar({ clubName, clubs = [], onClub, scope, teams, seasons, matches, rivals, players = [], goalkeepers = [], referencePreset, referenceScope, mode, onScope, onReferencePreset, onReferenceScope, onMode, onRefresh, fixture, comparison, comparisonEnabled = true, onComparisonEnabled }: DashboardFilterBarProps) {
  ...
  if (scope.scoreState !== "ALL") chips.push({ key: "score-state", label: scope.scoreState === "LEADING" ? "Ganando" : scope.scoreState === "DRAWING" ? "Empatando" : "Perdiendo", clear: () => patch({ scoreState: "ALL" }) });
  ...
  const clearAll = () => patch({ matchIds: [], period: "ALL", venues: [], results: [], rivals: [], phases: [], playerIds: [], goalkeeperIds: [], originZones: [], targetZones: [], outcomes: [], outcomeGroup: "ALL", originDistance: "ALL", competitiveContext: "ALL", playingState: "ALL", scoreState: "ALL" });
  ...
  return <><div data-dashboard-sticky-context className="sticky top-16 z-30 overflow-hidden rounded-b-3xl border border-t-0 border-slate-700 bg-slate-950/95 shadow-2xl backdrop-blur sm:rounded-3xl"><section className="p-3">
    ...
    <div className="mt-2 grid gap-2 lg:grid-cols-[1fr_auto]">
      ...
      <div className="mt-3 space-y-3">
        ...
        <div className="flex flex-wrap gap-2"><span className="w-full text-[9px] font-black text-slate-500" title="Filtra por la situación instantánea del marcador desde la perspectiva de CDA.">ESTADO MARCADOR</span>{(["ALL", "LEADING", "DRAWING", "TRAILING"] as const).map((value) => <MultiButton key={value} active={scope.scoreState === value} onClick={() => patch({ scoreState: value })}>{value === "ALL" ? "TODOS" : value === "LEADING" ? "GANANDO" : value === "DRAWING" ? "EMPATANDO" : "PERDIENDO"}</MultiButton>)}</div>
...
function ReferenceBuilder({ preset, scope, analysis, matches, rivals, onChange }: { preset: DashboardReferencePreset; scope: DashboardScopeV2; analysis: DashboardScopeV2; matches: SearchableMatch[]; rivals: string[]; onChange: (scope: DashboardScopeV2) => void }) {
  ...
  return <details open data-reference-builder className="rounded-2xl border border-amber-900/70 bg-amber-950/20 p-3"><summary className="cursor-pointer text-[10px] font-black text-amber-200">REFERENCIA PERSONALIZADA · {scope.rivals.length === 1 ? `VS ${scope.rivals[0]}` : scope.rivals.length > 1 ? `${scope.rivals.length} RIVALES` : scope.competition}</summary><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <label className="text-[9px] font-black text-slate-400">ESTADO MARCADOR<select aria-label="Estado del marcador de referencia" value={scope.scoreState} onChange={(event) => patch({ scoreState: event.target.value as DashboardScopeV2["scoreState"] })} className="mt-1 min-h-11 w-full rounded-xl bg-slate-900 px-3 text-xs text-white"><option value="ALL">Todos</option><option value="LEADING">Ganando</option><option value="DRAWING">Empatando</option><option value="TRAILING">Perdiendo</option></select></label>
