...
export function FacedMetricRow({ label, own, rival, ownReference, rivalReference, semantics = "neutral" }: { label: string; own: number | null; rival: number | null; ownReference: number | null; rivalReference: number | null; semantics?: "higher" | "lower" | "neutral" }) {
  ...
  return <div className="grid grid-cols-[3rem_minmax(4rem,1fr)_6rem_minmax(4rem,1fr)_3rem] items-center gap-2">
    ...
    <div className="relative h-5 rounded-l-full bg-slate-950"><span className="absolute right-0 top-0 h-full rounded-l-full bg-cyan-400" style={{ width: `${(own ?? 0) / max * 100}%` }} />{ownReference !== null && <i className="absolute bottom-[-2px] top-[-2px] z-10 border-l-2 border-dashed border-amber-200" style={{ right: `${ownReference / max * 100}%` }} />}</div>
    ...
    <div className="relative h-5 rounded-r-full bg-slate-950"><span className="absolute left-0 top-0 h-full rounded-r-full bg-rose-400" style={{ width: `${(rival ?? 0) / max * 100}%` }} />{rivalReference !== null && <i className="absolute bottom-[-2px] top-[-2px] z-10 border-l-2 border-dashed border-amber-200" style={{ left: `${rivalReference / max * 100}%` }} />}</div>
