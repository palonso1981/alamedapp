...
export function FacedMetricRow({ label, own, rival, ownReference, rivalReference, ownGoals, rivalGoals, semantics = "neutral" }: { label: string; own: number | null; rival: number | null; ownReference: number | null; rivalReference: number | null; ownGoals?: number; rivalGoals?: number; semantics?: "higher" | "lower" | "neutral" }) {
  ...
  return <div className="grid grid-cols-[minmax(6rem,1fr)_5rem_minmax(6rem,1fr)] items-center gap-2 sm:gap-3">
    <div className="relative h-8 rounded-l-full bg-slate-950"><span className="absolute right-0 top-1 h-6 rounded-l-full bg-cyan-400" style={{ width: `${ownWidth}%` }} /><strong data-bar-value="outside" data-bar-side="left" className="absolute top-1/2 z-20 -translate-y-1/2 whitespace-nowrap text-base font-black text-cyan-100" style={comparisonBarValueStyle("left", ownWidth)}>{format(own)}</strong>{ownReference !== null && <i title={`Referencia ${format(ownReference)}`} aria-label={`Referencia ${format(ownReference)}`} className="absolute bottom-[-2px] top-[-2px] z-10 border-l-2 border-dashed border-amber-200" style={{ right: `${ownReferencePosition}%` }}><b className="absolute -top-3 -translate-x-1/2 text-[8px] not-italic text-amber-100">{format(ownReference)}</b></i>}{ownGoals !== undefined && <span title={`${label} CDA · Remates ${format(own)} · Referencia ${format(ownReference)} · GF ${ownGoals} · % gol ${format(own && own > 0 ? ownGoals / own * 100 : null, "%")}`} aria-label={`GF ${ownGoals}`} className="absolute top-0 z-20 -translate-x-1/2 rounded-full border border-emerald-200 bg-emerald-500 px-1 text-[8px] font-black text-slate-950" style={{ right: `${comparisonBarPercentage(ownGoals, max)}%` }}>GF {ownGoals}</span>}</div>
