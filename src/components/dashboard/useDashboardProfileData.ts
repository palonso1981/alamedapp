...
export function useDashboardProfileData(pathname: string, area: DashboardArea = "PLAYERS") {
  ...
  return { analysis, clubs, currentClubId, fixture, matches, mode, query, ready, records, reference, referencePreset, rivals, scope, seasons, setCurrentClub, setMode, setReferencePreset, setScope, teams, workspace, refresh: () => setRecords(fixture ? buildDashboardFixture() : localRecords()) };
}
