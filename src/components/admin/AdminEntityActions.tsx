  ...
  return <>
    ...
    {confirmDelete && <div role="dialog" aria-modal="true" aria-label={`Eliminar ${label}`} className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/80 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-3xl border border-red-700 bg-slate-900 p-5 shadow-2xl">
        <p className="text-xs font-black uppercase tracking-wider text-red-300">Eliminar de APP ALAM</p>
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-2">
          ...
          <button type="button" onClick={() => { onDelete(); setConfirmDelete(false); }} className="min-h-12 rounded-xl bg-red-500 px-3 text-sm font-black text-white">ELIMINAR DE APP ALAM</button>
        </div>
      </div>
    </div>}
  </>;
