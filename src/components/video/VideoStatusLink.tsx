...
export function VideoStatusLink({ session, eventId, compact = false }: { session: MatchSession; eventId: string; compact?: boolean }) {
  ...
  if (resolution.status === "RESOLVED") {
    return <a href={resolution.url} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-10 items-center justify-center rounded-xl bg-red-600 px-3 text-xs font-black text-white" title={`Abrir ${resolution.leadSeconds}s antes · ${resolution.quality}`}>▶{compact ? "" : " YOUTUBE"}</a>;
  }
