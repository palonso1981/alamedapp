...
interface Props { session: MatchSession; segment: VideoLabSyncSegment; currentSecond: () => number; initialClip?: MatchVideoAnalysisClip; onSave: (clip: MatchVideoAnalysisClip) => void; onCancel: () => void; }

export function VideoClipComposer({ session, segment, currentSecond, initialClip, onSave, onCancel }: Props) {
  const initial = useMemo(() => initialClip ?? defaultVideoClipWindow(currentSecond()), [currentSecond, initialClip]);
  const [startSecond, setStart] = useState(initial.startSecond);
  const [endSecond, setEnd] = useState(initial.endSecond);
  const [category, setCategory] = useState(initialClip?.category ?? "");
  const [tagText, setTagText] = useState("");
  const [tags, setTags] = useState<string[]>(initialClip?.tags ?? []);
  const [playerIds, setPlayerIds] = useState<string[]>(initialClip?.playerIds ?? []);
  const [comment, setComment] = useState(initialClip?.comment ?? "");
  ...
  const save = () => onSave(createVideoAnalysisClip({ id: initialClip?.id, createdAt: initialClip?.createdAt, clubId: session.preparation?.clubId ?? "", matchId: session.matchId, segmentId: segment.id, videoId: segment.videoId, referenceSecond: initial.referenceSecond, startSecond, endSecond, category, tags, playerIds, comment }));

  return <div className="rounded-2xl border border-violet-700 bg-slate-950 p-3" role="dialog" aria-label="Crear clip de análisis">
    <div className="flex items-center justify-between"><div><p className="text-[10px] font-black text-violet-300">{initialClip ? "EDITAR CLIP" : "+ CLIP"} · REFERENCIA {formatVideoTimestamp(initial.referenceSecond)}</p><p className="text-xs text-slate-400">No modifica eventos ni estadísticas.</p></div><button type="button" onClick={onCancel} className="min-h-10 rounded-lg bg-slate-800 px-3 font-black">×</button></div>
    ...
    <button type="button" onClick={save} className="mt-3 min-h-12 w-full rounded-xl bg-violet-400 font-black text-slate-950">{initialClip ? "GUARDAR CAMBIOS" : "GUARDAR CLIP"}</button>
  </div>;
}
