...
export function RecentEventsPanel({ events, timeline, players, staff, onDelete, onRestore, onPendingReview, onSave, onMoveWithinMinute }: RecentEventsPanelProps) {
  ...
  const pendingCount = events.filter((event) => event.pendingReview && event.type !== "lineup_initialized").length;
  ...
  return (
    <section className={`rounded-2xl border border-slate-800 bg-slate-900 p-2 shadow-xl ${expanded ? "fixed inset-2 z-[60] flex flex-col sm:inset-6" : ""}`} aria-label="Cronología completa">
      ...
      <div className={`${expanded ? "flex-1" : "directo-timeline-scroll max-h-40"} min-h-0 overflow-y-auto rounded-xl border border-slate-800`}>
        <div className="sticky top-0 z-10 grid grid-cols-[44px_minmax(0,1fr)_auto] bg-slate-950 px-2 py-1 text-[9px] font-bold uppercase text-slate-500 xl:grid-cols-[58px_110px_minmax(0,1fr)_110px]"><span>Min</span><span className="hidden xl:block">Tipo</span><span>Acción</span><span className="hidden text-right xl:block">Revisar</span></div>
        ...
        {visible.map((event) => {
          ...
          return (
            <div key={event.id} data-event-id={event.id} className={`grid min-h-11 grid-cols-[44px_minmax(0,1fr)_auto] items-center border-t px-2 py-1 text-xs xl:grid-cols-[58px_110px_minmax(0,1fr)_110px] ${dropTarget?.eventId === event.id ? "border-cyan-300 bg-cyan-950" : "border-slate-800"} ${deleted ? "opacity-45" : ""}`}>
              <span className="font-mono font-black text-cyan-300">P{event.period} {String(event.minute).padStart(2, "0")}&apos;</span>
              <span className="hidden truncate text-[10px] font-bold uppercase text-slate-500 xl:block">{event.type.replaceAll("_", " ")}</span>
