...
interface GoalTargetPickerProps {
  ...
  onSelect: (point: GoalTargetCoordinates) => void;
  compact?: boolean;
}
...
export function GoalTargetPicker({ value, onSelect, compact = false }: GoalTargetPickerProps) {
  ...
  return (
    <div className="relative mx-auto w-full" data-testid="goal-target-picker">
      ...
      >
        <GoalFrameGraphic />
        {value && <TargetMarker point={value} />}
      </button>
    </div>
  );
}
...
export function KeeperBodyPicker({
  onSelect,
  target,
}: {
  onSelect: (part: KeeperBodyPart) => void;
  ...
  return (
    <div className="relative mx-auto aspect-[25/16] w-full overflow-hidden rounded-2xl border border-sky-300/30 bg-slate-950 shadow-inner" data-testid="keeper-body-picker">
      <GoalFrameGraphic />
      {target && <TargetMarker point={target} muted />}
      <div
        ...
        aria-label="Toca la parte corporal con la que interviene el portero"
        data-testid="goalkeeper-silhouette"
      >
      <svg viewBox="0 0 100 100" className="pointer-events-none absolute left-[10%] top-[14.5833%] h-[72.9167%] w-[80%] drop-shadow-[0_4px_8px_rgba(0,0,0,0.65)]" aria-hidden="true">
      ...
      {Object.entries(KEEPER_BODY_HITBOXES).map(([part, hitbox]) => (
        <button
          key={part}
          type="button"
...
function TargetMarker({ point, muted = false }: { point: GoalTargetCoordinates; muted?: boolean }) {
  return <span className={`pointer-events-none absolute z-10 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full border-[3px] border-white shadow-[0_0_0_6px_rgba(255,255,255,0.2)] ${muted ? "bg-slate-500/70" : "bg-rose-500"}`} style={{ left: `${point.x * 100}%`, top: `${point.y * 100}%` }} />;
}

function GoalFrameGraphic() {
  return <svg viewBox="0 0 100 64" preserveAspectRatio="none" className="pointer-events-none absolute inset-0 h-full w-full" aria-hidden="true">
    <defs><linearGradient id="goal-surface" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#081525" /><stop offset="1" stopColor="#123653" /></linearGradient></defs>
    ...
    <g stroke="#64748b" strokeWidth="0.45" opacity="0.55">{[27, 34.7, 42.3, 50, 57.7, 65.3, 73].map((x) => <line key={`v-${x}`} x1={x} y1="19" x2={x} y2="55" />)}{[24, 30, 36, 42, 48].map((y) => <line key={`h-${y}`} x1="24" y1={y} x2="76" y2={y} />)}</g>
